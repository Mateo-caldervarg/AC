...
# ╔═╡ 14db0a5c-2d48-4486-b8b8-1de63c78c137
using Plots
...
function NewtonRaphson(f::Function, f′::Function, Xo::Number, tol::AbstractFloat = 1e-5, iter::Int64 = 100) 
	g(Xo) = Xo - (f(Xo) / f′(Xo))
    [(Xo = g(Xo)) for i in 1:iter if abs(g(Xo)- Xo) > tol]
end


# ╔═╡ 3f422e50-8e2b-4da2-b50f-b6fb9c4bb2e0
NewtonRaphson(x -> 0.1*x^3 + x^2 -5 1, y -> 0.3*y^2 + 2*y, 2)
...
function derivadanumerica(f, x, h=10000)
	h * (f(x+(1/h)) - f(x))
end
...
# ╔═╡ bcb3e072-0150-49db-94c7-840816c68978
begin 
	x0 = 0.005:0.1:10
	f(x0) = log(x0)
	y0 = f.(x0)
	gr()
	plot(x0, y0, label="false")
	plot!(x, y, label=false)
end

# ╔═╡ cf003fbc-229a-4ba9-b36f-ee524c9213d9
function NewtonRaphson(f, f′, Xo, ϵ) 
	x1 = 0
	error = ϵ + 1
	while error > ϵ
		x1 = x0 - f(x0)/fd(x0)
...
# ╔═╡ dde31bfe-07d0-4103-a5a4-45305223e0d2
NewtonRaphson(x -> log(x) - 2, y -> 1/y, 8)

# ╔═╡ 339d6090-a4a2-40d5-8812-9bf0a801ee66
function NewtonDN(f, Xo, ϵ) 
	fd = derivadanumerica(f)
	x1 = 0
	error = ϵ + 1
	while error > ϵ
		x1 = x0 - f(x0)/fd(x0)
...
# ╔═╡ c6b8d4a1-9bd0-467f-a198-a1aab10d4ae3
begin
	u = -1:0.1:1.5
	k(u) = 7.38902*x-0.2
	p = k.(u)
...
# ╔═╡ 3ed43f77-362b-4f2b-9dcb-595ada73fd15
begin 
	x = -1:0.1:1.5
	h(x) = 7.36447*x-0.2
	k(x) = 7.38902*x-0.2
